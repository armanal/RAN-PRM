...
class PeakResponseMapping(nn.Sequential):
    ...
    def _patch(self):
        ...
        for i in self[0].attention:
            i.requires_grad = False

    def eval(self):
        for i in self[0].attention:
            i.requires_grad = False
        return super(PeakResponseMapping, self).eval()
    ...
    def forward(self, input, class_threshold=0, peak_threshold=3, retrieval_cfg=None):
        ...
        # print("Class Response Map shape :",class_response_maps.shape)
        ...
        if self.inferencing:
            ...
            class_threshold = 0
            peak_threshold = 0
            ##### class threshold by mean of aggregation #####
            # for idx in range(peak_list.size(0)):
            # 	class_threshold += aggregation[peak_list[idx, 0], peak_list[idx, 1]]
            # class_threshold = class_threshold // peak_list.size(0)
            ##### class threshold by median of aggregation #####
            ag = aggregation[peak_list[:, 0], peak_list[:, 1]]
            view = 1
            for i in ag.size():
            	view *= i
            class_threshold, _ = torch.median(ag.view(view), dim=0)
            print("class_threshold = ", class_threshold)

            ##### peak threshold by mean of aggregation #####
            for idx in range(peak_list.size(0)):
            	if aggregation[peak_list[idx, 0], peak_list[idx, 1]] >= class_threshold:
            		peak_threshold += class_response_maps[peak_list[idx, 0], peak_list[idx, 1], peak_list[idx, 2], peak_list[idx, 3]]
            peak_threshold = peak_threshold // peak_list.size(0)
